...
int hybrj(int(* fcndernn)(int n, const double *x, double *fvec, double *fjac, int ldfjac, int iflag, const double* para),
		  int n, double *x, double *fvec, const double* para, double *fjac, int ldfjac, double xtol, int
...
/*     and calculate its norm. */
	printf("%5s%6s%9s%11s\n", "iter.","nfev","f(x)","step");
    iflag = fcndernn(n, &x[1], &fvec[1], &fjac[fjac_offset], ldfjac, 1, para);
    *nfev = 1;
    if (iflag < 0) {
	goto TERMINATE;
    }
...
/*        calculate the jacobian matrix. */
        ...
        iflag = fcndernn(n, &x[1], &fvec[1], &fjac[fjac_offset], ldfjac, 2, para);
        ++(*njev);
        if (iflag < 0) {
            goto TERMINATE;
        }
...
/*           if requested, call fcn to enable printing of iterates. */
            ...
            if (nprint > 0) {
                iflag = 0;
                if ((iter - 1) % nprint == 0) {
                    iflag = fcndernn(n, &x[1], &fvec[1], &fjac[fjac_offset], ldfjac, 0, para);
...
/*           evaluate the function at x + p and calculate its norm. */
            ...
            iflag = fcndernn(n, &wa2[1], &wa4[1], &fjac[fjac_offset], ldfjac, 1, para);
            ++(*nfev);
            if (iflag < 0) {
                goto TERMINATE;
            }
...
/*     termination, either normal or user imposed. */
    ...
    if (nprint > 0)
	{
		fcndernn(n, &x[1], &fvec[1], &fjac[fjac_offset], ldfjac, 0, para);	
		printf("%4d%7d%12.3e%12.3e\n", iter, *nfev, enorm(n, &fvec[1]), delta);
    }
    return info;
