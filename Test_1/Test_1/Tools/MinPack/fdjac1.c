...
int fdjac1(int(* fcnnn)(int n, const double *x, double *fvec, int iflag, const double* para),
		   int n, double *x, const double *fvec, const double* para, double *fjac, int ldfjac, int ml, 
...
/*        computation of dense approximate jacobian. */
        ...
        for (j = 1; j <= n; ++j) {
            ...
            iflag = fcnnn(n, &x[1], &wa1[1], 1, para);//将倒数第2个参数由2改为1，jiangfh, 20140514
            if (iflag < 0) {
                return iflag;
            }
            x[j] = temp;
...
/*        computation of banded approximate jacobian. */
	...
	iflag = fcnnn(n, &x[1], &wa1[1], 1, para);
	if (iflag < 0) {
            return iflag;
	}
	for (j = k; msum < 0 ? j >= n : j <= n; j += msum) {
