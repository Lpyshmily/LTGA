...
* [7th version](#20201203-7th-version)
...
* 服务器运行中（种群数量Np=40，500代）
    ...
    100代结果：[0.753598593938617,0.0286820046901706,0.452880259549997,0.53176937506306,0.0188275899110854,0.604635126118764]，剩余质量为15967.622156118764kg。

    本机验证成功。

* 从《深空探测动力学与控制》上的已知最优解中计算6个优化变量的值，进行验证

    [0.87866987832683485,0.0,0.49933729465074800,0.77578570211868492,-0.034558202455132053,0.52316997261028420]，剩余质量为16021.641910375671kg。

    本机验证成功，而且很容易收敛。

    但是第5个优化变量超出了所给的变量范围，原因是第一段末端速度的3个分量为[0.055157140423736983,-0.10691164049102642,0.0046339945220568290]，而我们认为假定的3个分量的范围都是[-0.1,0.1]。

## 20201203 7th version

* 6变量PSO搜索

    扩大3个速度分量的范围为[-0.2,0.2]，进行PSO搜索，间接法最大猜测次数为500，种群数量为40，500代，并行运行时间为78287秒，平均1代157秒。

    最终结果为[0.863655768384999,0.000017319770127,0.499034175810391,0.621295996122853,0.228521973614765,0.515503534531755]，剩余质量为16021.090445199869kg，与已知最优解非常接近。

    本机验证成功，而且很容易收敛。

    在200代时，剩余质量已经达到16020.6173749935kg，后续提升很小。

* NLOPT局部优化

    最终结果为[0.877981487570480,0.000016618904766,0.499350791718267,0.637126883089754,0.232537206114017,0.511778825211389]，剩余质量为16021.632368045333kg。

    本机验证成功。

    到此，剩余质量与用间接法求解17个打靶的结果相比，仅差0.01kg，可以认为得到了最优解。