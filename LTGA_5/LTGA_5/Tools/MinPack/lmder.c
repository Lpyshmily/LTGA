...
int lmder(int (* fcndermn)(int m, int n, const double *x, double *fvec, double *fjac, int ldfjac, int iflag, const double* para),
		  int m, int n, double *x, double *fvec, const double* para, double *fjac, int ldfjac, double ftol,
...
/*     and calculate its norm. */
    ...
    iflag = fcndermn(m, n, x, fvec, fjac, ldfjac, 1, para);
    *nfev = 1;
    if (iflag < 0) {
	goto TERMINATE;
    }
...
/*        calculate the jacobian matrix. */
        ...
        iflag = fcndermn(m, n, x, fvec, fjac, ldfjac, 2, para);
        ++(*njev);
        if (iflag < 0) {
            goto TERMINATE;
        }
...
/*        if requested, call fcn to enable printing of iterates. */
        ...
        if (nprint > 0) {
            iflag = 0;
            if ((iter - 1) % nprint == 0) {
                iflag = fcndermn(m, n, x, fvec, fjac, ldfjac, 0, para);
				if(iter==1)
						printf("%5s%6s%9s%11s\n", "iter.","nfev","f(x)","step");
					printf("%4d%7d%12.3e%12.3e\n",iter-1,*nfev,enorm(m, fvec), delta);
            }
            if (iflag < 0) {
                goto TERMINATE;
            }
...
/*           evaluate the function at x + p and calculate its norm. */
            ...
            iflag = fcndermn(m, n, wa2, wa4, fjac, ldfjac, 1, para);
            ++(*nfev);
            if (iflag < 0) {
                goto TERMINATE;
            }
...
/*     termination, either normal or user imposed. */
    ...
    if (nprint > 0)
	{
		fcndermn(m, n, x, fvec, fjac, ldfjac, 0, para);
		printf("%4d%7d%12.3e%12.3e\n", iter-1, *nfev, enorm(m, fvec), delta);
    }
    return info;
