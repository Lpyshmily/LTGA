...
int lmdif(int(* fcnmn)(int m, int n, const double *x, double *fvec, int iflag, const double* para),
		  int m, int n, double *x, double *fvec, const double* para, double ftol, double xtol, double
...
/*        calculate the jacobian matrix. */
        ...
        iflag = fdjac2(fcnmn, m, n, x, fvec, para, fjac, ldfjac,
                       epsfcn, wa4);
        *nfev += n;
        if (iflag < 0) {
            goto TERMINATE;
...
/*           evaluate the function at x + p and calculate its norm. */
            ...
            iflag = fcnmn(m, n, wa2, wa4, 1, para);
            ++(*nfev);
            if (iflag < 0) {
                goto TERMINATE;
            }
