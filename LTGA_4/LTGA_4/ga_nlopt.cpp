...
void GA_PSO_6_new()
{
	...
	D = 6;
	Np = 40;
	PSO(GA_obj_PSO_6_new, xbest, fbest, NULL, D, Np, 500, 1);
	for (int i=0;i<D;++i)
		printf("xbest[%d]=%.15f,\n", i, xbest[i]);
	printf("fbest=%.15f\n", fbest);
}
...
{
	// double x_test[6] = {0.863655768384999,0.000017319770127,0.499034175810391,0.621295996122853,0.228521973614765,0.515503534531755}; // 16021.090445199869
	double x_test[6] = {0.877981487570480,0.000016618904766,0.499350791718267,0.637126883089754,0.232537206114017,0.511778825211389};
	printf("obj返回值：%f\n", GA_obj_nlopt_6_new(6, x_test, NULL, NULL));
}
...
void GA_nlopt_6_new()
{
	...
	nlopt_opt opter = nlopt_create(NLOPT_LN_COBYLA, 6);
	nlopt_set_min_objective(opter, GA_obj_nlopt_6_new, NULL);
	nlopt_set_lower_bounds(opter, lb);
	nlopt_set_upper_bounds(opter, rb);
	nlopt_set_xtol_rel(opter, tol);
