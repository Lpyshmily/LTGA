...
int fvec_ee_top_rend_fixed(int n, const double *x, double *fvec, int iflag, const double* sfpara)
{
	int i;
	...
	eef[5] = fmod(eef[5], M_2PI);

	V_Minus(fvec, x0, eef, 6);
	fvec[6] = x0[13];
	fvec[7] = V_Norm2(x, 8) - 1.0;
	fvec[8] = ham_ee_top(x0, lam0);
...
int solve_ee_top_rend_fixed(double* Out, const double* ee0, const double* eef, double m0, int MaxGuessNum)
{
	int n, j, info, flag=0;
	...
	while(num<=MaxGuessNum)
	{
		...
		info = hybrd1(fvec_ee_top_rend_fixed, n, x, fvec, sfpara, wa, xtol, 20, 2000);
		if(info>0 && enorm(n,fvec)<1e-8 && x[0]>0.0)
		{
			sfpara[13]=1.0;
			j=fvec_ee_top_rend_fixed(n, x, fvec, 1, sfpara);
