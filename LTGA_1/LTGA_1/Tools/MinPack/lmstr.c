...
/*     and calculate its norm. */
    ...
    iflag = fcnderstrmn(m, n, x, fvec, wa3, 1, para);
    *nfev = 1;
    if (iflag < 0) {
	goto TERMINATE;
    }
...
/*        if requested, call fcn to enable printing of iterates. */
        ...
        if (nprint > 0) {
            iflag = 0;
            if ((iter - 1) % nprint == 0) {
                iflag = fcnderstrmn(m, n, x, fvec, wa3, 0, para);
				if(iter==1)
						printf("%5s%6s%9s%11s\n", "iter.","nfev","f(x)","step");
...
/*        n components in qtf. */
        ...
        iflag = 2;
        for (i = 0; i < m; ++i) {
            if (fcnderstrmn(m, n, x, fvec, wa3, iflag, para) < 0) {
                goto TERMINATE;
            }
            temp = fvec[i];
...
/*           evaluate the function at x + p and calculate its norm. */
            ...
            iflag = fcnderstrmn(m, n, wa2, wa4, wa3, 1, para);
            ++(*nfev);
            if (iflag < 0) {
                goto TERMINATE;
            }
...
/*     termination, either normal or user imposed. */
    ...
    if (nprint > 0)
	{
		fcnderstrmn(m, n, x, fvec, wa3, 0, para);
		printf("%4d%7d%12.3e%12.3e\n", iter-1, *nfev, enorm(m, fvec), delta);
    }
    return info;
